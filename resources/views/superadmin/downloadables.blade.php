...
<body>
    <nav class="sidebar" id="sidebar">
        ...
        <ul class="nav-menu">
            ...
            <li class="nav-item">
                <a href="{{ route('superadmin.downloadables') ?? '#' }}" class="nav-link active">
    ...
    <div id="downloadableModal" class="modal">
        <div class="modal-content">
            ...
            <form id="downloadableForm" method="POST" action="{{ route('superadmin.downloadables.save') }}" enctype="multipart/form-data">
                @csrf
...
<script>
    ...
    async function deleteDownloadable(id) {
        ...
        try {
            await postForm("{{ route('superadmin.downloadables.delete') }}", { id });
    ...
    async function markAsRead(btn, id) {
        try {
            await postForm("{{ route('superadmin.downloadables.markRead') }}", { id });
